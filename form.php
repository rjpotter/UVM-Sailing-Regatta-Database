...
<main class="sluggersForm">
    <section class="formheader">
        ...
        if($_SERVER["REQUEST_METHOD"] == 'POST') {
            ...
            if ($dataIsGood) {
                try {
                    $sql = 'INSERT INTO tblPSLForm (fldTimeStamp, fldVenue, fldName, fldDate, fldRegatta, fldConditions, fldBoatNotes, fldWork, fldNotWork, fldProtest)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
                    ...
                    if ($statement->execute($data)) {
                        ...
                        $to = $email;
                        $from = 'UVM Sailing <rjpotter@uvm.edu>';
                        $subject = 'UVM Sailing Regatta Submissions';
                        $mailMessage = '<p style="font: 12pt Arial, sans-serif;">Thank you for submitting a ';
                        $mailMessage .= 'regatta report.</p><p>Your info will help our team for years to come!<br>';
                        $mailMessage .= '<span style="color: #cc0000;">UVM Sailing Team</span></p>';
                        $mailMessage .= $Message;
